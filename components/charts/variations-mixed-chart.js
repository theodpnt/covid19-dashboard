...
const formatData = data => {
  ...
  if (data.some(h => h.deces)) {
    datasets.push({
      ...
      data: data.map(h => h.deces).slice(1).map((cumul, i) => cumul - data[i].deces || null),
  ...
  if (data.some(h => h.reanimation)) {
    datasets.push({
      ...
      data: data.map(h => h.reanimation).slice(1).map((cumul, i) => cumul - data[i].reanimation || null),
  ...
  if (data.some(h => h.gueris)) {
    datasets.push({
      ...
      data: data.map(h => h.gueris).slice(1).map((cumul, i) => cumul - data[i].gueris || null),
...
const VariationsMixedChart = ({data, height}) => (
  <Bar data={formatData(data)} options={options} height={height} />
)

VariationsMixedChart.defaultProps = {
  height: null
}

VariationsMixedChart.propTypes = {
  data: PropTypes.array.isRequired,
  height: PropTypes.number
}

export default VariationsMixedChart
